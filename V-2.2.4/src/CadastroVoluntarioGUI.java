...
public class CadastroVoluntarioGUI extends JFrame implements ActionListener {
    ...
    private Timer loadingTimer;  // Timer para animação de carregamento
    ...
    // Configuração do Timer para animação de "Carregando..."
    String[] loadingText = {"Carregando   ", "Carregando.  ", "Carregando.. ", "Carregando..."};
    loadingTimer = new Timer(300, new ActionListener() {
        int index = 0;
        @Override
        public void actionPerformed(ActionEvent e) {
            cadastrarButton.setText(loadingText[index % loadingText.length]);
            index++;
        }
    });
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == cadastrarButton) {
            ...
            try {
                ...
                cadastrarButton.setEnabled(false); // Desativa o botão durante o carregamento
                loadingTimer.start(); // Inicia a animação de carregamento

                voluntarios v = new voluntarios();
                v.setNomevol(nomeField.getText());
                v.setCpfvol(cpfField.getText().replaceAll("[^\\d]", ""));
                v.setEmailvol(email);
                v.setFonevol(foneField.getText().replaceAll("[^\\d]", ""));
                v.setSenhavol(new String(senhaField.getPassword()));

                new Thread(() -> {
                    try {
                        // Simulando operação de cadastro
                        new ecobancodao().cadastrarOuAtualizarVoluntario(v);
                        Thread.sleep(3000); // Simula tempo de resposta do banco
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(this, "Erro ao cadastrar voluntário: " + ex.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
                    } finally {
                        SwingUtilities.invokeLater(() -> {
                            loadingTimer.stop(); // Para a animação
                            cadastrarButton.setText("Cadastrar"); // Restaura o texto do botão
                            cadastrarButton.setEnabled(true); // Reativa o botão
